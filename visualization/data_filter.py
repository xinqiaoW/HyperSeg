...
from segment_anything.modeling.dataset import remove_trailing_digits
import argparse
...
def compute_spectral_purity(hsi_image, gt):
    """
    计算高光谱图像中特定区域的光谱纯度（一致性）
    
    参数:
        hsi_image: 高光谱图像，形状为 (C, H, W)
        gt: 地面真实标签或二值掩码，形状为 (H, W)
            如果是掩码：True/1 的位置表示目标区域
            如果是标签：需要先转换为二值掩码
    ...
    mask = (gt_flat > 0)  # Support any positive label, not just 1

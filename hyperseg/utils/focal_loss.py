...
def focal_loss(output, target, alpha=0.25, gamma=2, reduction='mean'):
    ...
    alpha = target * (2 * alpha - 1) + (1 - alpha)
    one_minus_probs = target * (1 - probs) + (1 - target) * probs

    focal_weight = torch.pow(one_minus_probs, gamma)

    # add focal weight to cross entropy
    ce_loss = F.binary_cross_entropy_with_logits(output, target, reduction='none')
    focal_loss = focal_weight * ce_loss * alpha
...
def dice_loss(
    ...
    """
    num_masks = float(inputs.shape[0])
...
def loss_masks(src_masks, target_masks, num_masks):
    ...
    num_masks: for API validaity
    """
    loss_dice = dice_loss(src_masks, target_masks)
    loss_focal = 20 * focal_loss(src_masks, target_masks)
